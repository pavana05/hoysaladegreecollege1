            ...
            <div className="space-y-4 mt-2">
              ...
              <button
                onClick={async () => {
                  ...
                  // Fetch semester fees
                  const { data: semFees } = await supabase.from("semester_fees").select("semester, fee_amount").eq("student_id", student.id).order("semester");
                  const { data: semPayments } = await supabase.from("fee_payments").select("semester, amount").eq("student_id", student.id);
                  ...
                  const semesterFees = (semFees || []).map((sf: any) => ({
                    semester: sf.semester,
                    fee_amount: Number(sf.fee_amount),
                    paid: paidBySem[sf.semester] || 0,
                  }));
                  // Fetch recent payments
                  const { data: rp } = await supabase.from("fee_payments").select("receipt_number, amount, payment_date, payment_method, semester").eq("student_id", student.id).order("created_at", { ascending: false }).limit(5);
                  const recentPayments = (rp || []).map((p: any) => ({
                  ...
                  const html = generateFeeReceiptHtml({
                    ...
                    rollNumber: student.roll_number,
                    courseName: student.courses?.name || "—",
                    semester: lastReceipt.semester,
                    paymentMethod: lastReceipt.payment_method || "Cash",
